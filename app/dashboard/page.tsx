...
//   // Effect to fetch dashboard data when user object is available
//   useEffect(() => {
//     if (!authLoading && user) { // Only fetch if auth is not loading and user is logged in
//       fetchDashboardData();
//     } else if (!authLoading && !user) {
//       // If auth is done loading and no user, redirect to sign-in (or home)
//       // This is a basic redirect, consider using useRouter for more control
//       // window.location.href = '/auth/signin'; // Or router.push('/auth/signin'); if you import useRouter
//     }
//   }, [user, authLoading]); // Depend on user and authLoading

//   const fetchDashboardData = async () => {
//     setDataLoading(true); // Set data loading to true at the start of fetch
//     try {
//       // Fetch company profile for the current user
//       const { data: companyData, error: companyError } = await supabase
//         .from('companies')
//         .select('*')
//         .eq('user_id', user?.id) // Ensure user.id is used
//         .single();

//       // Handle case where no company profile is found (PGRST116 is "no rows found")
//       if (companyError && companyError.code !== 'PGRST116') {
//         throw companyError; // Re-throw other types of errors
//       }

//       setCompany(companyData); // Set company (will be null if not found)

//       // Only proceed to fetch tenders and applications if a company profile exists
//       if (companyData) {
//         // Fetch tenders posted by this company with their application counts
...
//         // Fetch applications made by this company
...
//         // Calculate and set dashboard statistics
//         const totalTenders = tenderData?.length || 0;
//         const activeTenders = tenderData?.filter((t: Tender) => t.status === 'open').length || 0;
//         const totalApplications = applicationData?.length || 0;
//         const pendingApplications = applicationData?.filter((a: Application) => a.status === 'pending').length || 0;
...
//         });
//       } else {
//         // If no company data, reset stats and lists
//         setTenders([]);
//         setApplications([]);
//         setStats({
//           totalTenders: 0,
...
//     } finally {
//       setDataLoading(false); // Always set data loading to false at the end
//     }
//   };
...
//           <Avatar className="h-16 w-16 rounded-full overflow-hidden"> {/* Added rounded-full and overflow-hidden */}
//             <AvatarImage src={company.logo_url || `https://placehold.co/64x64/E0E7FF/4F46E5?text=${company.name.charAt(0).toUpperCase()}`} alt={company.name} />
...
//         <TabsList className="grid w-full grid-cols-2"> {/* Added grid layout for tabs */}
...
//               <Link href="/tenders/manage">View All</Link>
//             </Button>
//           </div>
...
//                         </div>
//                         {tender.budget !== null && ( // Check for null explicitly
//                           <div className="flex items-center">
...
//                         </div>
//                         {application.proposed_budget !== null && ( // Check for null explicitly
//                           <div className="flex items-center">
//                             <DollarSign className="h-4 w-4 mr-1 text-gray-500" />
